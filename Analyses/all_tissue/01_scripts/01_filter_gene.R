...
if(!is.na(blacklist_path)){
        blacklist <- read.table(blacklist_path, h=F)
}
...
if(!is.na(blacklist_path)){
Raw_counts_good_samples <- Raw_counts_good_samples[!rownames(Raw_counts_good_samples) %in% blacklist$V1,]
}
...
filtered_genes <- expressed_genes[-overexpressed_genes,]
...
#overexpressed_genes
write.table(data.frame(rownames(expressed_genes)[overexpressed_genes]),
            paste(Output_path,"/overexpressed_genes", sep = ""),
            row.names = F, quote = F, col.names = F)

#Clean counts
write.table(data.frame(expressed_genes),
